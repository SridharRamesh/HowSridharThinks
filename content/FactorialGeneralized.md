...
Put another way, the appropriate asymptotics here amount to $$x! = G(x) = \int_{t = 0}^{\infty} t^x e^{-t} dt = \lim_{n \to \infty} \int_{t = 0}^{\infty} t^x (1 - t/n)^n dt = $$ $$\lim_{n \to \infty} n \int_{t = 0}^{\infty} (nt)^x (1 - t)^n dt = \lim_{n \to \infty} n^{x + 1} \int_{t = 0}^{\infty} t^x (1 - t)^n dt =$$ $$ \lim_{n \to \infty} n^{x + 1} \int_{t = 0}^{n} t^x (1 - t)^n dt = \lim_{n \to \infty} n^{x + 1} \beta(x + 1, n + 1) = $$ $$ \lim_{n \to \infty} n^{x + 1} G(x) G(n) / G(x + n + 1)$$, which is to say, $$n^{x + 1} = G(x + n + 1)/G(n)$$. \[TODO: Justify commuting limits here\]

Note that the relationship between $$\Gamma$$ and $$\beta$$ comes from the convolution identity, that the total integral of a convolution is the product of the total integral of the convolution factors, applied to functions on the positive real line of the form $$t \mapsto t^n e^{-t}$$.