...
This can be constructed as the quadratic residue code of length 11 over Z_3.

(Also can be constructed from the (11, 5, 2) biplane; see below)
***
...
TODO: Explain also the Gleason–Prange theorem, which shows why the extended code obtained by adding a parity bit to the construction below has a transitive automorphism group.
...
Under the cyclic group, any set of size 2 is equivalent to {0, x} for some nonzero x, and also just as well equivalent to {0, -x}; thus, between the two of these, it is equivalent to something of the form {0, x} for a quadratic residue x. And any two sets of the for {0, x} for a quadratic residue x are equivalent under multiplication by some quadratic residue. Thus, the automorphism group of this code is transitive on 2-sets[^transitivity].

[^transitivity]: Note that this condition of being transitive on 2-sets is slightly weaker than being 2-transitive (in that it says we can send any set {a, b} to any set {a', b'}, but does not imply we can send the ordered pair (a, b) to the ordered pair (a', b'); it may be sent to (b', a') instead). Actually, the automorphism group of the perfect Golay code is in fact 2-transitive, and indeed furthermore 4-transitive, but that will have to be shown in another way.
...
(We can alternatively think of the nontrivial (i.e., size 4) codewords of the extended Hamming code as those subsets of 3-dimensional space over Z_2 which amount to planes. The fact that we can find two Hamming codes whose intersection is only the trivial codewords arises from the fact that there is a bijection from 2^3 to itself which does not send any plane to a plane. Such a bijection can be constructed as, for example, sending 000 and 111 to themselves, swapping 100 and 110, and commuting with cyclic rotation. That this does not send any plane to a plane is seen by the fact that the difference between the original point and transformed point includes the first bit only if the original point is one of 100 and 110; similarly for the cyclic rotations of these. Thus, for the differences to sum to zero over some set, the set must contain both of 100 and 110 if it contains either, and similarly for their cyclic rotations. This means our set must be some union of two among {100, 110}, its cyclic rotations, and the leftover points {000, 111}. But no two of these pairs have the same sum. Thus, no plane is sent to a plane.)
...
Let M be a d by d square matrix in mod 2 land which is unitary/orthogonal in the sense that M's inverse is its transpose. (This is equivalent to the assertion that every two rows have an even intersection, and every single row has an even Hamming weight). Let us furthermore suppose that each row and each column has Hamming weight 3 mod 4 but not equal to exactly 3 (i.e., each row has Hamming weight among 7, 11, 15…).
...
Note that each pair (v, M(v)) is equivalently a pair (M^t(w), w) for w = M(v), with M^t satisfying the same conditions as M. So if a codeword with weights (a, b) among its two halves exists, so does one with weights (b, a) among the two halves. By explicit requirement, no codeword exists with weights (1, 3) among the two halves. And weight 0 in the first half entails weight 0 in the second half. So the only way to get a codeword of weight exactly four would be if a codeword existed with weights 2 in each half. This would mean two distinct rows of M (or rows of M^t; i.e., columns of M) who sum to weight 2. These two rows (or columns) would have to have the same weight w among 7, 11, 15, …, and overlap in precisely w - 1 positions out of those.

So let us now add an extra condition that this doesn't happen: No two rows or columns of M with the same weight intersect in all but one of their 1s each. That is, our conditons on M are that each row and column has Hamming weight 3 mod 4 but not exactly 3, every two distinct rows have an even intersection, and no two rows with the same Hamming weight intersect in one less than that weight (in other words, for any two rows, at least one of them has at least two values outside the other one). And same for columns. (Note that all rows having odd weight and any two rows having even intersection implies the same properties for columns.)
...
This will apply to both the dodecahedral and the (11, 5, 2) biplane constructions of the Golay code, like so:

(Actually, the above describes any Golay code permuted such that its first twelve bits can be independently chosen (i.e., they comprise a "dodecad" of the Golay code; note that the symmetry group of the Golay code is transitive on dodecad sets. TODO: Correct this parenthetical comment. This is conflating two different notions of "dodecad". The automorphism group is transitive on weight 12 codewords, not on sets of 12 bits that can be independently chosen. "Dodecad" seems to usually mean the former, not the latter). For any such code, we may express it in reduced row echelon form and will get a matrix consisting of an identity matrix next to a 12x12 matrix satisfying the above conditions. But such matrix conditions are easy to see directly in our (11, 5, 2) and dodecahedron constructions (as these are directly concerned with this 12x12 submatrix or something close to it), and not particularly easy to see directly in our other constructions.)
