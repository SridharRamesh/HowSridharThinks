...
First of all, there is the observation that GL has the uniform Craig interpolation property: That is, from any formula $$\phi(p, X)$$, there is another formula $$\exists p . \phi(p, X)$$, satisfying the adjunction: $$\exists p . \phi(p, X) \vdash \psi(X)$$ if and only if $$\phi(p, X) \vdash \psi(X)$$, for any formula $$\psi(X)$$ whose free variables are entirely among those in $$X$$. \[We can also consider the mirror image property that there is a formula $$\forall p . \phi(p, X)$$ such that $$\psi(X) \vdash \forall p . \phi(p, X)$$ iff $$\psi(p, X) \vdash \phi(X)$$. The two mirror image properties are equivalent in a Boolean context, by de Morgan dualizing\]. Uniform Craig interpolation for Boolean GL was proven by Shavrukov; originally in their dissertation, which is hard to find a copy of, but also explained in their paper ["Subalgebras of diagonalizable algebras of theories containing arithmetic"](https://eudml.org/doc/219353). This is by something like considering the finitely many possibilities up to suitable equivalence for unfolding a tree (as in the tree semantics of GL) to any particular finite depth over any particular finite set of propositional variables, so to speak. \[Both sides of intuitionistic uniform Craig interpolation seem to have also been announced as recently established by [van der Giessen et al](https://proof2025.ugent.be/talks/workshop/invited/invited-IvdGiessen_abstract.pdf), but the proof does not seem to have been published.\]
