...
How do we finally get from $$\Pi(x)$$ to $$\pi(x)$$? Well, keep in mind, $$\Pi(x) = \sum_{n \in \mathbb{N}^+} \frac{\pi(x^{1/n})}{n}$$. This relationship has such a nice form that we can recover $$\pi$$ from $$\Pi$$ using [Möbius inversion](../MoebiusInversion.html). From this, we find that $$\pi(x) = \sum_{n \in \mathbb{N}^+} \frac{\mu(n) \Li(x^{1/n})}{n}$$, where the Möbius function $$\mu$$ takes the value $$-1$$ at primes and zero at higher prime powers, and the Möbius function at a general positive integer is the product of the Möbius function at its prime power factors.
