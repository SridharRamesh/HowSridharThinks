...
Note that by rescaling, we have that $$c_n$$ is $$n^(-1 - p) c_{\signum{n}}$$. Furthermore, $$c_{-1}$$ is TODO times $$c_1$$ by TODO.

TODO: Extend now to the Fourier series for $$\Re(p) \in (1, 2)$$ [and indeed, $$\Re(p) > -1$$ in general]. Conclude by looking at any particular interval where $$\Re(p) > 1$$ that we have the Hurwitz functional equation, of which the Riemann functional equation is a special case.
